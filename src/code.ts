...
figma.ui.onmessage = async msg => {
  ...
  // Look for a 'Log' frame...
  if (widget) log = widget.findOne(node => node.type === "FRAME" && node.name == "Log") as FrameNode;
...
async function setUp() {
  if (!log) {
    ...
    widget.name = "Widget";
    if (otherWidgets) widget.x = otherWidgets[otherWidgets.length-1].x + 480
